...
while k<N:
    try:
        PRICE.append(load(RTSI[k], dtformed(3, 1, 2017), dtformed(3, 1, 2020),step=10))
        k=k+1
    except:
        continue
    finally:
...
# Расчёт индекса
def getindex(PRICE):
    indexat030119 = 1147.29	 # Значение индекса на 1-й торговый день 2017 года
    s=list()
    I=list()
    L=len(PRICE[0])
    for j in range(L):
...
X=list(range(1,L+1))
Y=getindex(PRICE)
[a, b]=linMMQ(X[0:int(2/3*L)],Y[0:int(2/3*L)])
...
plt.title("Индекс RTSI за 2017-2019 годы по месяцам")
plt.xlabel("Месяцы")
plt.grid()
plt.show()
